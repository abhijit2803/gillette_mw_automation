...
/**
 * Get Gillette Labs test credentials
 * @returns {{ username: string, password: string, pingId: string, isEncrypted: boolean }} Gillette credentials
 */
export function getGilletteCredentials() {
  const users = loadTestData('users.json');
  const gilletteCreds = users.testCredentials['gillette-dev'];
  
  return {
    username: process.env.GILLETTE_USERNAME || gilletteCreds.username,
    password: process.env.GILLETTE_PASSWORD || gilletteCreds.password,
    pingId: process.env.GILLETTE_PING_ID || gilletteCreds.pingId,
    isEncrypted: gilletteCreds.isEncrypted || false
  };
}
