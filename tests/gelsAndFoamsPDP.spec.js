// @ts-check
/**
 * Gels & Foams PDP Sanity Test Suite
...
// Product URL to test
const PRODUCT_URL = 'https://www.gillette.de/de-de/produkte/gesichtspflege/series-beruhigendes-rasiergel';

// Test Configuration
const TEST_CONFIG = {
  facebookExpectedMessage: 'https://www.gillette.de/',
  buyNowPopupMessage: 'Online-Händler',
  reviewButtonTitle: 'Bewertung schreiben'
};

test.describe('Gels & Foams PDP Sanity Tests - Germany Website', () => {
  ...
  test.beforeAll(async () => {
    ...
    log(SYMBOLS.ROCKET, '═══════════════════════════════════════════════════════════');
    log(SYMBOLS.ROCKET, 'GELS & FOAMS PDP SANITY TEST SUITE - GERMANY');
  ...
  test.afterAll(async () => {
    try {
      ...
      const reportsDir = path.join(process.cwd(), 'test-results', 'gels-foams-reports');
      ...
      const reportPath = path.join(reportsDir, `Gels_Foams_Sanity_Report_${timestamp}.html`);
  ...
  /**
   * Complete PDP Sanity Test for Gels & Foams Product
   */
  test('Complete PDP Sanity Test', async ({ page, context }) => {
    ...
    if (featureResult.success) {
      htmlReport.addStep("Click 'Feature Section' from MENU list", 'PASS', 
        `✅ 'Feature Section' option is clicked from the MENU List and page scrolls to FEATURE section. The first feature heading is: ${featureResult.featureTitle}`);
    } else {
      htmlReport.addStep("Click 'Feature Section' from MENU list", 'FAIL', "❌ 'Feature Section' not clicked.");
    }
    log(SYMBOLS.SUCCESS, '✅ TC - 8 PASSED: Feature Section navigation verified');
    ...
    if (reviewResult.success) {
      htmlReport.addStep("Click 'Review Section' from MENU list", 'PASS', 
        "✅ 'Review Section' option is clicked from the MENU List and page scrolls to REVIEW section.");
    } else {
      htmlReport.addStep("Click 'Review Section' from MENU list", 'FAIL', "❌ 'Review Section' not clicked.");
    }
    log(SYMBOLS.SUCCESS, '✅ TC - 9 PASSED: Review Section navigation verified');
 ...
 */
test.describe('Gels & Foams - Quick Sanity Test', () => {
